...
| „`/compact` mi 51× narazil na strop" | **Ani jednou.** Ve 21 zaznamenaných kompaktacích je trigger **21× `manual`, 0× `auto`**. Vždycky jsem `/compact` napsal sám, při mediánu 583 578 tokenů — tedy na ~58 % okna, dávno před stropem. Problém není že kompaktuju, ale **kdy** a **bez instrukce**. |
...
a `preservedSegment`. **21 zaznamenaných kompaktací** (alzask + fhb).
...
> Proto se 21 kompaktací nerovná 51× `/compact` z historie příkazů.
...
po `/compact`, byl **583 578 tokenů** (změřeno, 21 kompaktací). Vstup Opus 5 stojí
...
**Já se do auto-kompaktace nikdy nedostal** — 21 z 21 kompaktací bylo ručních (změřeno).
...
177,5 s (viz níže). Na starou cache `/clear` nesahá, ta prostě vyprší po TTL.
...
(`compactMetadata`, 21 kompaktací, `_raw/doklady-kompaktace.md`):

| Metrika | Hodnota (změřeno) |
|---|---:|
| Zaznamenaných kompaktací | 21 |
| **Ručních (`/compact`) / automatických** | **21 / 0** |
| Z toho s instrukcí (`/compact <co zachovat>`) | **0 / 21** |
| Medián kontextu **před** kompaktací | **583 578 tokenů** |
| Maximum před kompaktací | 618 390 tokenů |
| Medián kontextu **po** kompaktaci | **12 127 tokenů** |
| **Medián zahozeného podílu** | **97,3 %** |
| Rozsah zahozeného | 85,0 % – 98,7 % |
| **Medián doby kompaktace** | **177,5 s** |
| Nejdelší kompaktace | 224,1 s |
| **Celkem zahozeno** | **11 700 496 tokenů** |
...
**(3) Kompaktace trvá skoro tři minuty.** Medián **177,5 s**, nejdelší 224,1 s (změřeno).
...
**(4) 11 700 496 zahozených tokenů.** Číslo, které funguje samo. Jedenáct a sedm desetin milionu
...
583 578 → 12 127. Věta: *„To ‚97 % zahozeno‘ není moje interpretace. To je číslo, které si
Claude Code sám zapsal do transkriptu, včetně toho, jak dlouho to trvalo."*

**Výhrady, obě řekni:** (a) 21 kompaktací je **podvýběr** — transkripty pokrývají jen část
...
jednadvaceti neudělal."* Pak ukaž na **51 : 1** a na **177,5 s**.
...
| „`/compact` kontext zmenší." | **Shrne.** Medián: 583 578 → 12 127 tokenů, tedy **97,3 % zahozeno** (změřeno). Session ti zachrání (deck má pravdu) a zároveň nevíš, co vypadlo. |
...
| „`/compact` je rychlý příkaz." | Medián **177,5 s**, nejdelší 224,1 s (změřeno). Posílá další request přes celou historii (dokumentováno). `/clear` neposílá nic — je okamžitý a zdarma. |
...
při mediánu 583 578 tokenů nesl **každý** tah ~$2,90 vstupu bez cache (odhad z měřené velikosti
...
**Doklad:** 21 kompaktací, **21× manual / 0× auto, 0× s instrukcí** (změřeno). Medián
**583 578 → 12 127 tokenů = 97,3 % zahozeno**. Celkem **11 700 496 zahozených tokenů**.
...
`preTokens` → `postTokens` naživo (583 578 → 12 127) — *„to není moje interpretace, to si zapsal
Claude Code sám."*
**Priorita:** must
...
funguje dnes; a 21 kompaktací je **podvýběr** z transkriptů na disku, ne úplný počet — obojí říct.
...
`/compact` posílá sumarizační request přes celou historii — odtud medián **177,5 s**. Padesátkrát
...
A důležitá korekce vlastní diagnózy: **0 z 21 kompaktací bylo automatických** — vždycky jsem
...
neposílá request (dokumentováno) vs. medián `/compact` 177,5 s, nejdelší 224,1 s (změřeno).
21/21 manual, medián 583 578 tokenů před zásahem, auto-kompaktace by přišla až kolem 967K
...
**Co ukážu na obrazovce:** Tři čísla na prázdném slidu, nic jiného: **51 : 1** — **177,5 s** —
